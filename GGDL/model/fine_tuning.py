...
class FixedFeatureExtractor(Tuner):
    ...
    def layer_key_check_and_get_target_index(self)->int:
        ...
        if self.layer_key is None:
            raise ValueError("layer_key에 None이 입력되었습니다. layer_key는 header layer의 시작 이름의 일부로 구성되어야 합니다.")

        target_idx = self.find_before_extractor()
        if target_idx is None:
            raise ValueError(f"입력된 layer_key인 {self.layer_key}를 포함하는 parameter name이 존재하지 않습니다. self.idx_df를 통해 parameter들의 name을 확인하십시오.")
...
class PartialLayerFreezing(Tuner):
    ...
    def check_freezing_ratio(self):
        if self.freezing_ratio is None:
            raise ValueError("freezing_ratio에 None이 입력되었습니다. freezing_ratio는 float을 입력해야합니다.")
