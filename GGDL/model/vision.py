...
import torch, timm, re, warnings, os
from torch import nn
from huggingface_hub import hf_hub_download
from GGUtils.utils.module import GithubHelper
from GGUtils.utils.path import check_file_path_use_only_txt, file_download
...
class Classification(torch.nn.Module):
    def __init__(
        ...
        if TimmHelper.checker(model_name=model_name):
            self._set_backbone(model_name, pretrained, class_size, channel, global_pooling, custom_head_fn)
    ...
    def _set_backbone(
            self, model_name:str, pretrained:bool, class_size:Optional[int], 
            channel:Optional[int], global_pooling:Optional[str],
        ...
        self.backbone = timm.create_model(
            ...
            in_chans=channel, num_classes=class_size,
...
class TimmHelper:
    ...
    @classmethod
    def search(cls, pattern:str, match_exact:bool=False, ignore_case:bool=True)->List[str]:
        """
        ...
        # 결과가 하나도 없는 경우 문구 출력
        if len(result) == 0:
            warnings.warn("timm에 찾는 모델이 존재하지 않습니다. self.model_list로 전체 모델 확인이 가능합니다.")
        return result
    ...
    @classmethod
    def download_parameter_from_hf(cls, repo_id:str, param_file:str="pytorch_model.bin")->str:
        """
        ...
        try:
            ...
            raise ConnectionError(f"Parameter 다운로드 실패! Hugging face repo: '{repo_id}'가 다운로드 되지 않았습니다!") from e
    

    @classmethod
    def download_architecture_from_github(
            cls, 
            url:str, parents_path:str, github_token:Optional[str]=None, 
